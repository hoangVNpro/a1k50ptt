...
const App: React.FC = () => {
  ...
  // --- Handlers (Keep logic same, update visual feedback only) ---
  ...
  const handleOrderAction = async (orderId: string, action: 'completed' | 'delete') => {
    ...
    try {
      if (action === 'delete') await remove(ref(database, `orders/${orderId}`));
      else await update(ref(database, `orders/${orderId}`), { status: 'completed' });
    } finally {
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        <header className="flex flex-col md:flex-row justify-between items-center mb-8 md:mb-12 gap-6">
          ...
          <button 
            ...
            className={`w-full md:w-auto px-8 py-3 rounded-full border-2 text-sm font-bold transition-all shadow-lg transform hover:scale-105 active:scale-95 flex items-center justify-center gap-2 ${
              isAdminMode 
              ? 'bg-yellow-400 border-yellow-400 text-red-900 hover:bg-yellow-300' 
        ...
        {isAdminMode ? (
          <div className="space-y-8 animate-[fadeIn_0.5s_ease-out]">
              ...
              <form onSubmit={handleAddProduct} className="space-y-4">
                ...
                <div className="flex flex-col md:flex-row gap-4">
                  ...
                  <input 
                    type="file" 
                    accept="image/*"
                    className="flex-1 bg-red-900/40 border border-yellow-500/20 rounded-xl p-3 file:mr-4 file:py-1 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-yellow-500 file:text-red-900 hover:file:bg-yellow-400 text-yellow-200"
            ...
            {/* Orders */}
            <div className="glass-panel rounded-3xl p-6 md:p-8 border-yellow-500/30">
              <h2 className="text-2xl font-bold mb-6 flex items-center gap-2 text-yellow-300 font-header">
                <i className="fas fa-list-ul"></i> Danh Sách Đơn Hàng
              </h2>
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse whitespace-nowrap">
                  <thead>
                    <tr className="text-yellow-500/60 border-b border-yellow-500/20 text-sm uppercase">
                    ...
                    {orders.length === 0 ? (
                      ...
                      orders.map(order => (
                        <tr key={order.id} className="border-b border-yellow-500/10 hover:bg-yellow-500/5 transition-colors">
                          ...
                          <td className="p-4 font-bold text-yellow-100">{order.customerName}</td>
                          ...
                          <td className="p-4">
                             {order.status === 'pending' ? <span className="text-yellow-300 bg-yellow-900/40 px-2 py-1 rounded border border-yellow-500/30 text-xs">Chờ xử lý</span> : <span className="text-green-400 bg-green-900/40 px-2 py-1 rounded border border-green-500/30 text-xs">Hoàn thành</span>}
                          </td>
                          <td className="p-4 flex justify-center gap-2">
                            {order.status === 'pending' && <button onClick={() => handleOrderAction(order.id, 'completed')} className="w-8 h-8 rounded-full bg-green-500/20 text-green-400 hover:bg-green-500 hover:text-white flex items-center justify-center"><i className="fas fa-check"></i></button>}
                            <button onClick={() => handleOrderAction(order.id, 'delete')} className="w-8 h-8 rounded-full bg-red-500/20 text-red-400 hover:bg-red-500 hover:text-white flex items-center justify-center"><i className="fas fa-trash"></i></button>
                          </td>
                        </tr>
                      ))
                    )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6 animate-[fadeIn_0.5s_ease-out]">
            {loading ? (
              <div className="col-span-full text-center py-20">
                ...
                <p className="text-yellow-200">Đang tải dữ liệu Tết...</p>
              </div>
            ) : products.length === 0 ? (
              ...
              products.map((product) => (
                <div 
                  ...
                  className="glass-panel rounded-2xl overflow-hidden group cursor-pointer hover:-translate-y-2 transition-transform duration-300 hover:shadow-[0_0_30px_rgba(251,191,36,0.2)] border border-yellow-500/20 hover:border-yellow-400"
                >
                  ...
                  <div className="p-4 relative">
                    ...
                    <div className="flex justify-between items-center">
                      <span className="font-bold text-lg text-yellow-400 text-glow">
                        {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(product.price)}
                      </span>
                      <button className="w-8 h-8 rounded-full bg-yellow-500/20 text-yellow-400 flex items-center justify-center hover:bg-yellow-500 hover:text-red-900 transition-colors border border-yellow-500/30">
